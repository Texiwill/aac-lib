...
function vexpert_login() {
	...
	then
		...
		password="" # clear just to be sure, we may have different passwords
		username="" # and usernames
		handlecredstore
		vauth=$auth
		# just to be sure
...
$0 [-c|--check] [--clean] [--dlg search] [--dlgl search] [-d|--dryrun] [-f|--force] [--fav favorite] [--favorite] [--fixsymlink] [-e|--exit] [-h|--help] [--historical] [-mr] [-nh|--noheader] [--nohistorical] [--nosymlink] [-nq|--noquiet] [-ns|--nostore] [-nc|--nocolor] [--dts|--nodts] [--oem|--nooem] [--oss|--nooss] [--oauth] [-p|--password password] [--progress] [-q|--quiet] [--rebuild] [--symlink] [-u|--username username] [-v|--vsmdir VSMDirectory] [-V|--version] [--verify] [--veriforce] [-y] [-z] [--debug] [--repo repopath] [--save] [--olde 12] [--timeout <number>]
    ...
    -o|--output - specify the location to write the licenses file
...
function getLicensesEval() {
	...
	do
		...
		echo $lic_list | jq '.result.productName+"\t"+(.result.licenses[]|.serialKey+"\t"+.expirationDate)' | sed 's/\\t/\t/g'|sed 's/"//g' 2>&1 | tee $licenseoutput
	done
}
...
getLicenses() {
	...
	then
		if [ $noheader -eq 0 ]
		then 
			colorecho "	Licenses:	1"
			colorecho "	Output:	$licenseoutput"
		fi
		...
		colorecho "Licenses written to $licenseoutput"
	fi
}
...
if [ $liceout -ne 1 ]
then
	licenseoutput=$cdir/licenses.txt
fi
